...
class _EcranPlanBusState extends State<EcranPlanBus> {
  ...
  // A la fin des 5 minutes : on demande au client s'il est toujours la,
  // au lieu de liberer silencieusement le siege
    ...
    _minuteurConfirmation = Timer.periodic(const Duration(seconds: 1), (timer) {
      setState(() {
        if (_secondesConfirmation > 0) {
          ...
          timer.cancel();
          // Pas de reponse en 10s -> annulation de la selection
  ...
  void _confirmerPresence() {
    ...
    _demarrerMinuteur(); // relance 5 minutes completes
  }
  ...
  void _selectionnerSiege(String numero, StatutSiege statut) {
    ...
    // Reclic sur le siege deja selectionne -> on annule
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            // Voile + boite de confirmation "toujours la ?" en fin de minuteur
