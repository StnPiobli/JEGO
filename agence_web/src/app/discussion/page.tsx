  ...
  return (
    <LayoutAgence>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="rounded-2xl p-3 mb-4 bg-red/6 border border-red/20">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.auteur === 'agence' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[82%] rounded-2xl px-4 py-3 ${m.auteur === 'agence' ? 'bg-green-700 text-white rounded-br-sm' : 'bg-off-white text-ink rounded-bl-sm'}`}>
                  ...
                  <p className={`text-[10px] mt-2 ${m.auteur === 'agence' ? 'text-white/70' : 'text-ink-soft'}`}>{m.heure}</p>
                </div>
              </div>
            ))}
            ...
            {enAttente && (
              <div className="flex justify-start">
                <div className="bg-off-white rounded-2xl rounded-bl-sm px-4 py-3 flex gap-1">
          ...
          <form onSubmit={envoyer} className="border-t border-line p-4 space-y-3">
            {pieces.length > 0 && (
              <div className="flex gap-2 overflow-x-auto pb-1">
                {pieces.map((piece) => (
                  <div key={piece.id} className="relative shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-line bg-off-white">
                    {piece.type === 'image' ? (
                      <img src={piece.url} alt={piece.nom} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex flex-col items-center justify-center text-center p-2 text-[11px] text-ink">
                        <span className="text-lg">📄</span>
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 rounded-2xl bg-off-white border border-transparent focus-within:border-green-700 transition-colors px-4 py-3">
                ...
                <p className="text-[11px] text-ink-soft mt-1">Tu peux joindre des captures, photos, PDF ou autres documents pour JEGO.</p>
              </div>

              <button type="submit" className="w-11 h-11 rounded-xl bg-green-700 hover:bg-green-900 text-white flex items-center justify-center shrink-0 transition-colors">
