-- ═══════════════════════════════════════════════════
-- JEGO — Schéma de base de données
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 7 — RÔLES ↔ PERMISSIONS (liaison)
-- ═══════════════════════════════════════════════════
...
CREATE TABLE sieges (
  ...
  type_position VARCHAR(20) CHECK (type_position IN ('fenetre_gauche', 'couloir_gauche', 'milieu', 'couloir_droit', 'fenetre_droite')),
  est_premium   BOOLEAN DEFAULT FALSE,
  statut        VARCHAR(30) DEFAULT 'disponible' CHECK (statut IN ('disponible', 'desactive', 'supprime_toilettes')),
  cree_le       TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE trajets (
  ...
  categorie             VARCHAR(20) CHECK (categorie IN ('standard', 'vip', 'express', 'nuit')),
...
CREATE TABLE billets (
  ...
  siège_id            UUID NOT NULL REFERENCES sieges(id),
  agence_id           UUID NOT NULL REFERENCES agences(id),
  type_billet         VARCHAR(20) DEFAULT 'standard' CHECK (type_billet IN ('standard', 'flexible', 'cadeau')),
  ...
  source_vente        VARCHAR(20) DEFAULT 'en_ligne' CHECK (source_vente IN ('en_ligne', 'physique')),
  est_revente         BOOLEAN DEFAULT FALSE,
  billet_original_id  UUID REFERENCES billets(id),
  cree_le             TIMESTAMP DEFAULT NOW(),
  mis_a_jour_le       TIMESTAMP DEFAULT NOW()
  ALTER TABLE billets DROP COLUMN groupe_id;
  ALTER TABLE billets ADD COLUMN groupe_id UUID REFERENCES reservations_groupe(id);
);
...
-- ═══════════════════════════════════════════════════
-- TABLE 16 — SOFT LOCK (verrou temporaire sièges)
-- ═══════════════════════════════════════════════════
...
CREATE TABLE soft_locks (
  ...
  cree_le     TIMESTAMP DEFAULT NOW(),
  UNIQUE (siège_id, trajet_id)
);

-- ═══════════════════════════════════════════════════
-- TABLE 17 — REMBOURSEMENTS
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 21 — COMMENTAIRES ET NOTES
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 22 — JEGO POINTS — HISTORIQUE
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 25 — DEMANDES CRÉATION MEMBRE
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 27 — HISTORIQUE MODIFICATIONS PROFIL
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 30 — JOURNAL DE BORD QUOTIDIEN
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 34 — MODÈLES NOTIFICATIONS
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 35 — RAPPORTS MENSUELS AGENCE
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 36 — DEMANDES NOUVELLE BRANCHE
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 37 — CODES DOUBLE AUTHENTIFICATION
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- TABLE 38 — EXPORTS DONNÉES CLIENT
-- ═══════════════════════════════════════════════════
